    ...
    try:
        ...
        try:
            ...
            if mode != 't':
                ...
                os.makedirs(f'{output_path}/txt/{grade}', exist_ok=True)
                with open(f'''{output_path}/txt/{grade}/{xmlfile.split('/')[-1][:-5]}.txt''', 'w') as f:
                    f.write(text)
            
            # Tabular Data
            if mode != 'x':
                content = ['<content>']
                os.makedirs(f'{output_path}/table/{grade}/', exist_ok=True)
                ...
                with open(f'''{output_path}/table/{grade}/{xmlfile.split('/')[-1][:-5]}.xml''', 'a') as f:
...
def find_files(directory: str):
    ...
    for path, _, filenames in os.walk(directory):
        for file in filenames:
            if (os.path.isfile(f'{path}/{file}') and file.endswith('.html')):
...
if __name__ == '__main__':
    ...
    for path in (pbar:= tqdm(args.files)):
        ...
        if os.path.isdir(path):
            file_list.extend(find_files(path))
